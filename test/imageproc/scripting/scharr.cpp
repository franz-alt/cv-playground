...
TEST(test_scripting_algorithm_scharr, compile_valid_parameters)
{
    ...
    // good case
    {
        ...
        image_processor.compile(
            R"(
                ...
                var edges = scharr(input_rgb, 3, "hor", "ignore")
            )",
...
TEST(test_scripting_algorithm_scharr, compile_invalid_parameters)
{
    // create a thread pool for a single thread
    auto pool = boost::asynchronous::make_shared_scheduler_proxy<
                    boost::asynchronous::multiqueue_threadpool_scheduler<
    ...
    // case: invalid filter width (!= 3)
    {
        ...
        image_processor.compile(
            R"(
                var input_rgb = input("rgb", 8)
                var edges = scharr(input_rgb, 4, "hor", "ignore")
            )",
            [promise_compile](bool successful, std::size_t compile_id)
            {
                ASSERT_TRUE(!successful);
    ...
    // case: invalid operation mode
    {
        ...
        image_processor.compile(
            R"(
                var input_rgb = input("rgb", 8)
                var edges = scharr(input_rgb, 3, "horizontal", "ignore")
            )",
            [promise_compile](bool successful, std::size_t compile_id)
            {
                ASSERT_TRUE(!successful);
    ...
    // case: invalid border mode
    {
        ...
        image_processor.compile(
            R"(
                var input_rgb = input("rgb", 8)
                var edges = scharr(input_rgb, 3, "hor", "do some magic")
            )",
            [promise_compile](bool successful, std::size_t compile_id)
            {
                ASSERT_TRUE(!successful);
