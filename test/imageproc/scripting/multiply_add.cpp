...
TEST(test_scripting_algorithm_multiply_add, compile_valid_parameters)
{
    ...
    // good case
    {
        ...
        image_processor.compile(
            R"(
                ...
                var brighten = multiply_add(input_rgb, 3.1415, 128)
            )",
...
TEST(test_scripting_algorithm_multiply_add, compile_invalid_parameters)
{
    // create a thread pool for a single thread
    auto pool = boost::asynchronous::make_shared_scheduler_proxy<
                    boost::asynchronous::multiqueue_threadpool_scheduler<
    ...
    // case: invalid negativ offset
    {
        ...
        image_processor.compile(
            R"(
                var input_rgb = input("rgb", 8)
                var brighten = multiply_add(input_rgb, 3.1415, -666)
            )",
            [promise_compile](bool successful, std::size_t compile_id)
            {
                ASSERT_TRUE(!successful);
    ...
    // case: invalid positive offset
    {
        ...
        image_processor.compile(
            R"(
                var input_rgb = input("rgb", 8)
                var brighten = multiply_add(input_rgb, 3.1415, 256)
            )",
            [promise_compile](bool successful, std::size_t compile_id)
            {
                ASSERT_TRUE(!successful);
